...
| [`ENT:PushEnt`](push-ent.md) | Documentation pending. |
...
| [`ENT:_BaseInitialize`](base-initialize-137.md) | Documentation pending. |
| [`ENT:_BaseInitialize`](base-initialize-1910.md) | _BaseInitialize is an empty stub in DRGBase called after _InitModules() completes, so all internal fields (_DrGBaseSequenceEvents etc.) are already set up by the time this runs.  Overriding Initialize() directly would skip _InitModules() entirely. |
