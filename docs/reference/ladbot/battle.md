...
```lua
function ENT:PlayBattleStartAnim(cam, target, fov)
```
...
```lua
function ENT:StartBattle(target)
```
...
<p class="api-source">Defined in <code>lua/entities/lad_framework_base/battle.lua:26</code>.</p>
...
```lua
function ENT:StartDynamicIntroBattle(target, skipEarlySoundtrack)
```
